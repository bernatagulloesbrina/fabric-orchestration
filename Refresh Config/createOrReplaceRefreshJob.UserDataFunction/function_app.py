...
def _normalize_dependencies(jobName: str, precedentJobNames: Optional[str]) -> list[str]:
    # No dependencies provided -> valid refresh job with zero precedents
...
def create_or_replace_refresh(
    ...
    USING (SELECT ? AS job_name, 'Fabric' AS job_type, ? AS object_type) AS source
    ON target.job_name = source.job_name
    WHEN MATCHED THEN
        UPDATE SET job_type = source.job_type, object_type = source.object_type
    WHEN NOT MATCHED THEN
    ...
    insert_dependency_sql = "INSERT INTO dbo.refresh_job_precedence (job_name, precedent_job_name) VALUES (?, ?);"
    ...
    try:
        ...
        # Always clear previous dependencies first.
        cursor.execute(delete_dependencies_sql, normalized_job_name)

        # Reinsert only if a new dependency list was provided.
        if dependencies:
            _execute_many(
                cursor,
                insert_dependency_sql,
                [(normalized_job_name, d) for d in dependencies],
            )
    ...
    return f"Refresh '{normalized_job_name}' synchronized with {len(dependencies)} precedent job(s)."